...
## Now need to fix these for the new API
...
def __set_bond_property__(connectivity, bond, key, property):
    try:
        return connectivity.__set_property__(bond, key, property)
    except Exception as e:
        if e.__class__.__name__ == "ArgumentError":
            from ..legacy import Base as _Base
            return connectivity.__set_property__(bond, key,
                                                 _Base.wrap(property))
        else:
            raise e
