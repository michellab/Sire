...
static const RegisterMetaType<InternalFlipMove> r_internalflipmove;

/** Serialise to a binary datastream */
QDataStream &operator<<(QDataStream &ds, const InternalFlipMove &internalflipmove)
{
    writeHeader(ds, r_internalflipmove, 2);
    
    SharedDataStream sds(ds);
    
    sds << internalflipmove.smplr
        << internalflipmove.center_function
        << internalflipmove.flexibility_property
        << static_cast<const MonteCarlo&>(internalflipmove);
    
    return ds;
}

/** Extract from a binary datastream */
QDataStream &operator>>(QDataStream &ds, InternalFlipMove &internalflipmove)
{
    VersionID v = readHeader(ds, r_internalflipmove);
    ...
    if (v == 2)
    {
        ...
        sds >> internalflipmove.smplr
            >> internalflipmove.center_function
            >> internalflipmove.flexibility_property
            >> static_cast<MonteCarlo&>(internalflipmove);
    }
    else if (v == 1)
    {
        SharedDataStream sds(ds);
    
        internalflipmove.center_function = GetCOMPoint();
        ...
        throw version_error(v, "1", r_internalflipmove, CODELOC);

    return ds;
}

/** Null constructor */
InternalFlipMove::InternalFlipMove(const PropertyMap &map) 
             : ConcreteProperty<InternalFlipMove,MonteCarlo>(map)
{
...
InternalFlipMove::InternalFlipMove(const MoleculeGroup &molgroup, const PropertyMap &map)
             : ConcreteProperty<InternalFlipMove,MonteCarlo>(),
...
/** Construct the mover move that samples molecules from the
    passed sampler */
InternalFlipMove::InternalFlipMove(const Sampler &sampler, const PropertyMap &map)
             : ConcreteProperty<InternalFlipMove,MonteCarlo>(),
               smplr(sampler)
{
    flexibility_property = map["flexibility"];
...
/** Copy constructor */
InternalFlipMove::InternalFlipMove(const InternalFlipMove &other)
             : ConcreteProperty<InternalFlipMove,MonteCarlo>(other),
               smplr(other.smplr), center_function(other.center_function),
               flexibility_property(other.flexibility_property)
{}

/** Destructor */
InternalFlipMove::~InternalFlipMove()
{}

/** Copy assignment operator */
InternalFlipMove& InternalFlipMove::operator=(const InternalFlipMove &other)
{
    if (this != &other)
    {
...
/** Comparison operator */
bool InternalFlipMove::operator==(const InternalFlipMove &other) const
{
...
bool InternalFlipMove::operator!=(const InternalFlipMove &other) const
{
    return not InternalFlipMove::operator==(other);
}

/** Return a string representation of this move */
QString InternalFlipMove::toString() const
{
    return QObject::tr("InternalFlipMove( nAccepted() = %1 nRejected() == %2 )")
                            .arg( this->nAccepted() )
                            .arg( this->nRejected() );
}

/** Set the sampler used to sample molecules for this move */
void InternalFlipMove::setSampler(const Sampler &sampler)
{
    smplr = sampler;
    smplr.edit().setGenerator( this->generator() );
}
...
void InternalFlipMove::setCenterOfMolecule(const GetPoint &cf)
{
    center_function = cf;
}

/** Return the function used to find the center of the molecule */
const GetPoint& InternalFlipMove::centerOfMolecule() const
{
...
void InternalFlipMove::setGenerator(const RanGenerator &rangenerator)
{
...
/** Internal function used to set the ensemble based on the
    passed temperature */
void InternalFlipMove::_pvt_setTemperature(const Temperature &temperature)
{
    MonteCarlo::setEnsemble( Ensemble::NVT(temperature) );
}

static double clipDouble(double value)
{
...
void InternalFlipMove::move(System &system, int nmoves, bool record_stats)
{
    if (nmoves <= 0)
        return;

    InternalFlipMove old_state(*this);
...
const char* InternalFlipMove::typeName()
{
    return QMetaType::typeName( qMetaTypeId<InternalFlipMove>() );
}
